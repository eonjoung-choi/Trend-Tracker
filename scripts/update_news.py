...
# 단어 끝 조사 목록(긴 것부터 — "으로"가 "로"보다 먼저 매칭되도록)
_JOSA = ("으로", "에서", "에게", "와의", "과의", "이랑",
         "와", "과", "은", "는", "이", "가", "을", "를", "에", "의", "도", "로", "랑", "및")


def _strip_josa(w: str) -> str:
    """단어 끝 조사 제거(어간이 2글자 이상 유지될 때만). '솔라나와'→'솔라나', '재단과'→'재단'."""
    for j in _JOSA:
...
def is_duplicate_article(title_a: str, title_b: str) -> bool:
    ...
    #   ★ 조사(와/과/은/는/이/가/을/를/에/의/로 등)를 떼고 비교 →
    #     "솔라나와" 와 "솔라나"를 같은 단어로 인식해 동일 사건 중복을 잡음
    nouns_a = set(_strip_josa(w) for w in re.findall(r"[가-힣]{2,}", title_a))
    nouns_b = set(_strip_josa(w) for w in re.findall(r"[가-힣]{2,}", title_b))
    # 불용어 제거 (너무 흔한 단어)
